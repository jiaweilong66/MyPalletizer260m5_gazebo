...
```bash
$ cd ~/catkin_ws/src
$ git clone https://github.com/jiaweilong66/MyPalletizer260m5_gazebo.git
$ cd ~/catkin_ws
$ catkin_make
$ source devel/setup.bash
```
MyPalletizer260_gazebo-Gazebo使用说明
1. 滑块控制
...
```bash
source devel/setup.bash
roslaunch MyPalletizer260_gazebo slider.launch _port:=/dev/ttyACM0 _baud:=115200
```
...
```bash
source devel/setup.bash
rosrun MyPalletizer260_gazebo  robot.py _port:=/dev/ttyACM0 _baud:=115200
```
...
```bash
source devel/setup.bash
rosrun MyPalletizer260_gazebo robot1.py _port:=/dev/ttyACM0 _baud:=115200
```
...
```bash
source devel/setup.bash
roslaunch MyPalletizer260_gazebo teleop_keyboard.launch _port:=/dev/ttyACM0 _baud:=115200
```
...
```bash
source devel/setup.bash
rosrun MyPalletizer260_gazebo robot2.py _port:=/dev/ttyACM0 _baud:=115200
```

如果运行成功，我们将在终端看到如下的输出信息：

```shell
MyPalletizer260 Teleop Keyboard Controller
---------------------------
